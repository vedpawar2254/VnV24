  ...
  return (
    <section className="bg-primary py-16">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-serif text-center font-bold text-forest-green mb-8">Shop by Category</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
          {categories.map((category) => (
            <Link key={category.name} href={category.href} className="group relative aspect-square">
              <div className="absolute inset-0 rounded-lg overflow-hidden border border-border-color">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h3 className="text-xl md:text-2xl font-serif font-bold text-gold">{category.name}</h3>
                </div>
              </div>
            </Link>
          ))}
